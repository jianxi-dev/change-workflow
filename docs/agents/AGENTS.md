...
清单由 `../lib/render.sh:95-101` 的 glob 自动纳入 `cw_list_files`（该 glob 跳过 `AGENTS.md` 自身）；受管文件共 18 个 = 6 硬编码 + 12 份 docs（`../lib/render.sh:87`）。
...
- `cw_list_files` 是 glob，新增模板无需改受管清单（`../lib/render.sh:95-101`）。
- 但要更新 e2e 模板头计数断言（`../test/install-update-e2e.sh:192` 期望 13）。
