...
### 8. 质量反哺：引用纪律 / 决策日志 / 验证时效 / 减法审计（1.6.0）

**问题**：门禁齐备，但四处仍靠自觉——① 应用/豁免 QG/DQ 无记录要求，「提一句编号」即可敷衍过去；② 跨会话/隔夜运行没有可审计的决策轨迹（收尾只有 change-close-pending 信号）；③ rebase / 追加提交**静默作废** QG-5 判定（结论仍躺在票上「看起来有效」——最危险的失效形态）；④ 门禁只增不减，从未审计哪条真的在抓东西。

**设计**（每项都落在既有 gate，不新增 gate）：

| 项 | 机制 | 挂载 |
|---|---|---|
| 引用纪律 | PR body 逐条写 `QG-x / DQ-x: <具体决策>`；空引用不予合并 | SKILL.md G2 / quality-gates §七 / PR 模板 |
| 决策日志 | `scripts/decisions-log.sh` 每票一行 TSV（时间/阶段/决策/理由/证据/结果），默认本地不入库 | SKILL.md G3 |
| 验证时效 | QG-5 标注「验证基于 `<sha>`」；`pr-automation.sh --verified-sha` 与分支 HEAD 比对，不一致拒收 | SKILL.md G1/G2 / pr-automation.sh |
| 减法审计 | 季度「门禁捕获统计」+ 零捕获合并/退役；最小充分集免退役 | quality-gates §八 |

**关键取舍**：验证时效采用「记录验证时 SHA + 收口比对」而非比对基点——rebase 与追加提交都会重写/前移 HEAD，一个判据覆盖两种失效；检查置于 gh 校验之前（纯本地判定、无需网络/凭证、可确定性测试）。`--verified-sha` 缺失仅警告不阻塞：降级不改变门禁判据，拦截发生在「提供了且不符」时。
...
| **拒绝自我安装** | 20 个受管文件里 18 个的模板源与安装目标同路径；渲染先截断再读取 → 清空模板（实测 11913 字节 → 0），且会把模板记成受管基线，此后每次改模板都报冲突 |
| **消费仓滚动验证** | 工具包改动的影响面只在消费仓升级后才显形（1.1.5 覆盖定制、1.2.0 永不归一均由消费侧暴露）；CI 没有消费仓检出，故列为发布前本地门禁 |
