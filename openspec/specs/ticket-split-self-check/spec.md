# ticket-split-self-check Specification

## Purpose
拆票（G0-POST）发布前自检能力：以 `scripts/cw-tickets-check.sh`（C1-C8 机检）承接原 quiz 人工确认——子票与 tasks.md 对账双射、票面七字段、AC 用户可观测形态与禁入信号、Blocked by DAG 无环、豁免显式声明、规模钩子、粒度声明与票间重叠；不可机检的三项语义判断以书面自答落款 spec issue 留痕；逃逸由下游捕获（G1 拒开工 / DQ-7）触发判据修订。
## Requirements
### Requirement: 发布前机检门禁
