...
- `cw_refuse_symlink`（`../lib/render.sh:62`）：所有受管写入面（渲染重定向、`cp` 安装、manifest/conf 重写）必须先过这道闸，拒绝符号链接目标（C2 安全边界）。使用点：渲染目标 `:103`、cp 源/目标 `:175-176`、受管脚本 `:201`；`update.sh:132`/`:194`/`:258-259`/`:452-453`、`setup.sh:146`/`:221`。
