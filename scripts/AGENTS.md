...
本目录是模板源：4 个受管脚本 `pr-automation.sh` / `cw-update.sh` / `cw-evidence.sh` / `cw-greploop.sh` 都装到消费仓的 `scripts/`，清单唯一定义在 `../lib/render.sh:110` 的 `cw_list_files`。改这里的文件即改变所有消费仓的安装产物。
...
- 四件套门禁函数 `run_gate`（`:62`）；`--skip-checks` 是逃生舱，`../skills/change-workflow/SKILL.md:188` 明确禁止。
...
- `cw_refuse_symlink`（`../lib/render.sh:62`）：所有受管写入面（渲染重定向、`cp` 安装、manifest/conf 重写）必须先过这道闸，拒绝符号链接目标（C2 安全边界）。使用点：渲染目标 `:84`、cp 源/目标 `:132-133`、受管脚本 `:153`；`update.sh:171`/`:234-235`/`:378-379`、`setup.sh:146`/`:221`。
...
1. 新增/删除受管脚本 → 改 `../lib/render.sh:110` 的 `cw_list_files`。
2. 同步 `../test/install-update-e2e.sh`：脚本语法检查循环（`:202`）与 Python 裸 `$VAR` 检查列表（`:209`）—— **新增任何脚本（含测试侧）都要加进这两处**：本仓的「`$VAR` 紧邻全角字符」陷阱只有这里拦得住（1.2.1 的 `test/rollout-check.sh` 与 `update.sh` 守卫都曾踩中）。
